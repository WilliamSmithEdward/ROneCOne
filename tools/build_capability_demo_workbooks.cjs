...
const capabilities = [
  ...
  },
  {
    ...
    architecture: [
      ["Single-file core", "ROneCOne.cls", "One import", "ENFORCED", 1, 0],
      ["No new dependency", "Existing OLE DB path", "DbCommand and DbDataAdapter unchanged", "ENFORCED", 1, 0],
      ["Values are parameters", "? markers only", "A captured value can never be read as SQL", "ENFORCED", 1, 0],
      ["Inspectable", "ToSqlString", "The exact statement is visible before it runs", "ENFORCED", 1, 0],
      ["Refusal over fallback", "ROneCOne.QueryError", "Untranslatable expressions refuse, never scan locally", "ENFORCED", 1, 0],
    ],
    examples: [
      [
        "A filter becomes SQL",
        "Where(...).ToSqlString",
        "orders.Where(\"Total\").AtLeast(10).ToSqlString",
        "SELECT * FROM [Orders$] WHERE ([Total] >= ?)",
      ],
      [
        "The server does the filtering",
        "Where(...).Count",
        "orders.Where(\"Total\").AtLeast(10).Count",
        3,
      ],
      [
      ...
      [
        "Ordering and paging translate",
        "OrderBy(...).Take(n)",
        "orders.OrderBy(\"Id\").Take(2).ToDataTable.Rows.Count",
        2,
      ],
      [
        "Projection picks columns",
        "SelectColumns(...)",
        "orders.SelectColumns(\"Name\", \"Total\").Take(1) _\n    .ToDataTable.Columns.Count",
        2,
      ],
      [
        "A quote stays data, never SQL",
        "parameterized ? marker",
      ...
      [
        "A literal percent is escaped",
        "ContainsText(\"100%\")",
        "orders.Where(\"Name\").ContainsText(\"100%\").Count",
        1,
      ],
      [
  ...
  },
  {
    ...
    examples: [
      ...
      [
        ...
        "rows.Count(predicate)",
        "sales.Rows.Count( _\n    sales.Rows!Region.EqualTo(\"West\"))",
        3,
      ],
      [
        "Filter, sort, and project in one chain",
        "Where().OrderBy().Select()",
        "sales.Rows.Where( _\n    sales.Rows!Amount.AtLeast(100)) _\n    .OrderBy(\"Rep\").JoinText(\", \", \"Rep\")",
        "Ada, Cy",
      ],
      [
        "Take the top rows after sorting",
        "OrderByDescending().Take(2)",
        "sales.Rows.OrderByDescending(\"Amount\") _\n    .Take(2).Sum(\"Amount\")",
        320,
      ],
      [
        "Sort by two keys",
        "OrderBy().ThenByDescending()",
        "sales.Rows.OrderBy(\"Region\") _\n    .ThenByDescending(\"Amount\") _\n    .JoinText(\">\", \"Rep\")",
        "Bo>Dee>Cy>Ada>Eve",
      ],
      [
        "Distinct values of one column",
        "Select().Distinct().Order()",
        "sales.Rows _\n    .SelectItems(\"Region\", vbString) _\n    .Distinct.Order.JoinText(\", \")",
        "East, North, West",
      ],
      [
        "Group by a column, total each group",
        "GroupBy(...).Sum(...)",
        "Set grouped = sales.Rows.GroupBy(\"Region\")\ngrouped.First.Key & \" \" & _\n    grouped.First.Sum(\"Amount\")",
        "West 380",
      ],
      [
      ...
      [
        "Sorted, so the top row is the biggest",
        "view.Sort",
        "topWest.Item(0).Item(\"Rep\")",
        "Cy",
      ],
      [
        "Aggregate just the filtered rows",
        "view.Sum(...)",
        "topWest.Sum(\"Amount\")",
        380,
      ],
      ...
      [
        ...
        "objects.Where(...)",
        "objects.Where(objects _\n    .Condition(\"Region\").EqualTo(\"West\")).Count",
        3,
      ],
      [
      ...
      [
        "CSV writes the header row first",
        "CsvHelper-style writing",
        "Split(sales.ToCsv, vbCrLf)(0)",
        "Region,Rep,Amount",
      ],
      [
        "And CSV comes back as a table",
        "CsvHelper-style typed parsing",
        "ROneCOne.Csv.DeserializeTable( _\n    sales.ToCsv, \"FromCsv\").Rows.Count",
        5,
      ],
      [
